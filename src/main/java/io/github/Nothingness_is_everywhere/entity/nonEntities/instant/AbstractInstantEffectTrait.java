...
public abstract class AbstractInstantEffectTrait extends AbstractNonEntities implements InstantEffectTrait {
    protected int cooldown;              // 技能冷却（释放后进入冷却，与持续型冷却逻辑不同）
    protected int cost;                  // 消耗资源（如魔力值/怒气值，根据游戏设定）

    public AbstractInstantEffectTrait(String name, String description, int cooldown, int cost) {
    ...
    public abstract boolean hasEnoughResource(LifeTrait caster);
